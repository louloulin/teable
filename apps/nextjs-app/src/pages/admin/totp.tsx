...
const TotpAdminPage: NextPageWithLayout = () => (
  <EnterprisePlaceholderPage
    title="TOTP / 2FA"
    description="Per-user TOTP enrolment, factor revocation and backup code re-issue."
    cloudCapability="Cloud §admin-panel/totp"
    ossBackend="POST /api/auth/totp/enrollments, DELETE /api/auth/totp/factors/:id"
  />
);
...
TotpAdminPage.getLayout = function getLayout(page: ReactElement, pageProps) {
  return <AdminLayout {...pageProps}>{page}</AdminLayout>;
};

export default TotpAdminPage;
