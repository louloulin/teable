...
describe('frontend observability modules', () => {
  ...
  describe('error reporter', () => {
    ...
    it('reset clears the install marker', () => {
      ...
      (globalThis as { __teable_error_reporter_installed__?: boolean }).__teable_error_reporter_installed__ = true;
      __resetErrorReporterForTests();
      expect(
        (globalThis as { __teable_error_reporter_installed__?: boolean }).__teable_error_reporter_installed__
      ).toBeUndefined();
    });
  });
});
