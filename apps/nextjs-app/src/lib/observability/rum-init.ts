...
type OtelSdk = {
  WebTracerProvider: new (config: { resource: unknown; spanProcessors: unknown[] }) => {
...
export async function startRUM(): Promise<boolean> {
  ...
  try {
    ...
    // The disable comments below cover the ENTIRE statement that follows —
    // `import/no-unresolved` reports the error on the line with the module
    // string, which can be the line AFTER the directive for multi-line
    // `import(...)` calls; a block-scoped directive is the only way to be
    // certain.
    /* eslint-disable import/no-unresolved */
    apiMod =
      // prettier-ignore
      (await import(/* webpackIgnore: true */ '@opentelemetry/api')) as unknown as OtelApi;
    sdkMod =
      // prettier-ignore
      (await import(/* webpackIgnore: true */ '@opentelemetry/sdk-trace-web')) as unknown as OtelSdk;
    exporterMod =
      // prettier-ignore
      (await import(/* webpackIgnore: true */ '@opentelemetry/exporter-trace-otlp-http')) as unknown as OtelExporter;
    resourcesMod =
      // prettier-ignore
      (await import(/* webpackIgnore: true */ '@opentelemetry/resources')) as unknown as OtelResources;
