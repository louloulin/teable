...
function resolveEnvironment(): string {
  return process.env.DEPLOYMENT_ENVIRONMENT?.trim() || process.env.NODE_ENV?.trim() || 'development';
}
...
export function initTracing(): ShutdownHandle {
  if (g[GLOBAL_KEY]) {
    logger.log(`initTracing() called again — returning existing handle (mode=${g[GLOBAL_KEY].mode})`);
