...
export class AiStreamingService {
  ...
  async streamChunks(
    ...
    const controller = signal instanceof AbortController ? signal : undefined;
    const abortSignal: AbortSignal = controller?.signal ?? (signal as AbortSignal);
    ...
    try {
      for await (const chunk of chunks) {
        if (abortSignal.aborted || this.isStreamClosed(response)) {
          break;
        }
