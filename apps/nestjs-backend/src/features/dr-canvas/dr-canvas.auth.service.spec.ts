...
function canvas(): DrCanvasSpec {
  return {
    ...
    nodes: [
      ...
      { id: 't', kind: 'restore', ref: 'restore_snapshot', label: 'T', position: { x: 1, y: 0 }, config: {} },
    ],
...
describe('DrCanvasAuthService helpers', () => {
  it('addNode / removeNode / addEdge / removeEdge / moveNode', () => {
    const svc = setup();
    const c0 = canvas();
    const c1 = svc.addNode(c0, { id: 'r', kind: 'replicate', ref: 'incremental_replicate', label: 'R', position: { x: 0, y: 1 }, config: {} });
