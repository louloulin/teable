...
function canvas(over: Partial<DrCanvasSpec> = {}): DrCanvasSpec {
  return {
    ...
    nodes: [
      { id: 's', kind: 'snapshot', ref: 'pg_dump', label: 'S', position: { x: 0, y: 0 }, config: {}, checkpoint: { id: 'cp1', marker: 'lsn:100', takenAt: 1 } },
      { id: 'r', kind: 'replicate', ref: 'incremental_replicate', label: 'R', position: { x: 1, y: 0 }, config: {} },
      { id: 't', kind: 'restore', ref: 'restore_snapshot', label: 'T', position: { x: 2, y: 0 }, config: {} },
    ],
    edges: [
      { id: 'e1', from: 's', to: 'r' },
      { id: 'e2', from: 'r', to: 't' },
...
describe('dr-canvas.validateDrCanvas', () => {
  ...
  it('flags missing snapshot', () => {
    ...
    c.nodes.push({ id: 'r', kind: 'replicate', ref: 'incremental_replicate', label: 'R', position: { x: 1, y: 0 }, config: {} });
  ...
  it('flags duplicate ids', () => {
    const c = canvas();
    c.nodes.push({ id: 's', kind: 'snapshot', ref: 'pg_dump', label: 'S2', position: { x: 0, y: 1 }, config: {} });
    const v = validateDrCanvas(c);
...
describe('dr-canvas.addNode / removeNode / addEdge / removeEdge / moveNode', () => {
  it('addNode idempotent', () => {
    const c0 = canvas();
    const c1 = addDrNode(c0, { id: 'v', kind: 'verify', ref: 'verify_checksum', label: 'V', position: { x: 1, y: 1 }, config: {} });
    const c2 = addDrNode(c1, { id: 'v', kind: 'verify', ref: 'verify_checksum', label: 'V', position: { x: 1, y: 1 }, config: {} });
