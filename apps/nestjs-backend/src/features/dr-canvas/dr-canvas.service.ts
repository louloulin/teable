...
export function validateDrCanvas(
  ...
  if (canvas.nodes.length > MAX_DR_CANVAS_NODES) {
    issues.push({
      code: 'too_many_nodes',
      message: `nodes ${canvas.nodes.length} > ${MAX_DR_CANVAS_NODES}`,
    });
  }
  if (canvas.edges.length > MAX_DR_CANVAS_EDGES) {
    issues.push({
      code: 'too_many_edges',
      message: `edges ${canvas.edges.length} > ${MAX_DR_CANVAS_EDGES}`,
    });
  }
  ...
  for (const e of canvas.edges) {
    ...
    if (e.requiresCheckpoint && !checkpoints.has(e.requiresCheckpoint)) {
      issues.push({
        code: 'missing_required_checkpoint',
        target: e.id,
        message: `missing checkpoint ${e.requiresCheckpoint}`,
      });
    }
  }
  ...
  if (
    canvas.edges.some(
      (e) =>
        e.from === edge.from &&
        e.to === edge.to &&
        (e.requiresCheckpoint ?? '') === (edge.requiresCheckpoint ?? '')
    )
  ) {
    return canvas;
  }
...
export function findDrNodeByCheckpoint(
  canvas: DrCanvasSpec,
  checkpointId: string
): DrNodeSpec | undefined {
  return canvas.nodes.find((n) => n.checkpoint?.id === checkpointId);
}

/** List all checkpoints. */
export function listDrCheckpoints(
  canvas: DrCanvasSpec
): DrCanvasSpec['nodes'][number]['checkpoint'][] {
