...
describe('AutomationActionCatalogAuthService.get / list / group / index', () => {
  it('get / list / group / index', () => {
    const svc = setup();
    expect(svc.getCatalog().types.length).toBe(18);
    expect(svc.get('send_email')?.label).toBe('Send email');
    expect(svc.listByCategory('notification').length).toBe(3);
    expect(svc.groupByCategory()['notification'].length).toBe(3);
    expect(svc.index().size).toBe(18);
  });
});
...
describe('AutomationActionCatalogAuthService.extend / serialize / summarize / ping', () => {
  ...
  it('summarize', () => {
    const svc = setup();
    expect(svc.summarize().count).toBe(18);
  });
  it('ping', async () => {
    const svc = setup();
