...
export class NotionOAuthService {
  ...
  private async defaultLoadSetting(key: string): Promise<unknown> {
    ...
    const service = this.getSettingService(SettingService);
    if (!service) return null;
    const all = await service.getSetting([key]);
    return all[key];
  }
  ...
  private async defaultSaveSetting(key: string, value: unknown): Promise<unknown> {
    ...
    const service = this.getSettingService(SettingService);
    if (!service) return null;
    return service.updateSetting({ [key]: value } as Record<string, unknown>);
  }

  /** Resolve the singleton `SettingService` from the Nest module graph. */
  private getSettingService<T>(ctor: new (...args: unknown[]) => T): T | null {
    // The provider is registered globally via `SettingModule`; `app.get` is
    // available through `ModuleRef` in a real Nest context. For simplicity
    // we look it up lazily via the host module's reference — the controller
    // wires it directly when it needs the service.
    const ref = (this as unknown as { moduleRef?: { get: (t: unknown) => unknown } }).moduleRef;
    if (!ref) return null;
    return ref.get(ctor) as T;
  }
