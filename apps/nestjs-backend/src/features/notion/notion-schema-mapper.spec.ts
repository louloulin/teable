...
describe('mapNotionPropertyToField', () => {
  ...
  it('maps url to URL', () => {
    ...
    expect(mapping.skipReason).toBeUndefined();
    expect(mapping.fieldRo).toMatchObject({ name: 'Homepage', type: FieldType.SingleLineText });
  });

  it('maps email to Email', () => {
    const mapping = mapNotionPropertyToField('Contact', prop('email'));
    expect(mapping.skipReason).toBeUndefined();
    expect(mapping.fieldRo).toMatchObject({ name: 'Contact', type: FieldType.SingleLineText });
  });

  it('maps phone_number to PhoneNumber', () => {
    const mapping = mapNotionPropertyToField('Phone', prop('phone_number'));
    expect(mapping.skipReason).toBeUndefined();
    expect(mapping.fieldRo).toMatchObject({ name: 'Phone', type: FieldType.SingleLineText });
  });
