...
describe('WebhookDeliveryAuthService', () => {
  ...
  describe('dispatchOne', () => {
    it('marks delivered on 200', async () => {
      ...
      const r = await svc.dispatchOne({ delivery: mkDelivery() as unknown as Parameters<typeof svc.dispatchOne>[0]['delivery'] });
    ...
    it('marks dead after max attempts on 5xx', async () => {
      ...
      const r = await svc.dispatchOne({ delivery: mkDelivery({ attempt: 4 }) as unknown as Parameters<typeof svc.dispatchOne>[0]['delivery'] });
