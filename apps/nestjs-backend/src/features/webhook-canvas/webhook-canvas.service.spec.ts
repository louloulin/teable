...
function canvas(over: Partial<WebhookCanvasSpec> = {}): WebhookCanvasSpec {
  return {
    ...
    version: 1,
    nodes: [
      { id: 'src', kind: 'source', ref: 'event_source', label: 'S', position: { x: 0, y: 0 }, config: {} },
      { id: 'pick', kind: 'transform', ref: 'pick_fields', label: 'P', position: { x: 1, y: 0 }, config: {} },
      { id: 'u', kind: 'url', ref: 'http_post', label: 'U', position: { x: 2, y: 0 }, config: { url: 'https://x' } },
    ],
    edges: [
...
describe('webhook-canvas.validateWebhookCanvas', () => {
  ...
  it('flags duplicate ids', () => {
    const c = canvas();
    c.nodes.push({ id: 'src', kind: 'source', ref: 'event_source', label: 'S2', position: { x: 0, y: 1 }, config: {} });
    const v = validateWebhookCanvas(c);
...
describe('webhook-canvas.addNode / removeNode / addEdge / removeEdge / moveNode', () => {
  it('addNode idempotent', () => {
    const c0 = canvas();
    const c1 = addWebhookNode(c0, { id: 'r', kind: 'retry', ref: 'retry_block', label: 'R', position: { x: 1, y: 1 }, config: {} });
    const c2 = addWebhookNode(c1, { id: 'r', kind: 'retry', ref: 'retry_block', label: 'R', position: { x: 1, y: 1 }, config: {} });
...
describe('webhook-canvas.serialize / summarize / groupByKind', () => {
  ...
  it('groupByKind', () => {
    expect(Object.keys(groupWebhookNodesByKind(canvas())).sort()).toEqual(['source', 'transform', 'url']);
  });
});