...
function resolveRef(kind: WebhookNodeKind, ref: string, catalog?: Record<string, readonly string[]>): boolean {
  if (!ref) return false;
  if (catalog) {
...
export function validateWebhookCanvas(
  ...
  if (canvas.nodes.length > MAX_WEBHOOK_CANVAS_NODES) {
    issues.push({ code: 'too_many_nodes', message: `nodes ${canvas.nodes.length} > ${MAX_WEBHOOK_CANVAS_NODES}` });
  }
  if (canvas.edges.length > MAX_WEBHOOK_CANVAS_EDGES) {
    issues.push({ code: 'too_many_edges', message: `edges ${canvas.edges.length} > ${MAX_WEBHOOK_CANVAS_EDGES}` });
  }
...
export function addWebhookNode(canvas: WebhookCanvasSpec, node: WebhookNodeSpec): WebhookCanvasSpec {
...
/** Add an edge. */
export function addWebhookEdge(canvas: WebhookCanvasSpec, edge: WebhookEdgeSpec): WebhookCanvasSpec {
...
/** Move node. */
export function moveWebhookNode(canvas: WebhookCanvasSpec, id: string, position: { x: number; y: number }): WebhookCanvasSpec {
...
/** Summarize. */
export function summarizeWebhookCanvas(canvas: WebhookCanvasSpec): { nodes: number; edges: number; urls: number; transforms: number } {
  return {
...
/** Group nodes by kind. */
export function groupWebhookNodesByKind(canvas: WebhookCanvasSpec): Record<string, WebhookNodeSpec[]> {
