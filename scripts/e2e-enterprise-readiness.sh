...
# Cloud Business parity: 40/42 (api_rate_limit + dashboard no_rows_yet + baserow_r16 + clickup_r17 + jira_r18 + monday_r19 added).
...
assert_ok "$([[ "$PARITY_DEFAULT" == "40/42" ]] && echo 0 || echo 1)" \
  "default self_hosted parity = 40/42 (got: $PARITY_DEFAULT; api_rate_limit opt-out + dashboard no_rows_yet)"
...
ROUND5_KEYS="airtable_import notion_import google_sheets_import baserow_import clickup_import jira_import monday_import view_permission"
...
log "[OK]   all 8 round-5 wired migration/UI capabilities registered + enabled (airtable + notion + google_sheets + baserow_r16 + clickup_r17 + jira_r18 + monday_r19 + view_permission)"
...
# 4 driver_missing (framework present, no driver) -- 4 migration; baserow_r16 + clickup_r17 + jira_r18 + monday_r19 now implemented
...
")
assert_ok "$([[ "$DRIVER_MISSING" == "4" ]] && echo 0 || echo 1)" \
  "4 driver_missing gaps still pending driver (Round-19: monday moved to implemented) (got: $DRIVER_MISSING)"
...
assert_ok "$([[ "$MS_IMPL" == "7" ]] && echo 0 || echo 1)" \
  "migration-sources implemented == 7 (airtable + notion + google_sheets + baserow_r16 + clickup_r17 + jira_r18 + monday_r19) (got: $MS_IMPL)"
assert_ok "$([[ "$MS_PEND" == "4" ]] && echo 0 || echo 1)" \
  "migration-sources pending == 4 (nocodb/smartsheet/smartsuite/connect_more_sources) (got: $MS_PEND)"
...
all_partial = all(g['status'] == 'partial' for g in driver_missing if g['key'] not in ('baserow_import', 'clickup_import', 'jira_import', 'monday_import'))
...
assert_ok "$([[ "$MS_PARTIAL_CHECK" == "true" ]] && echo 0 || echo 1)" \
  "4 driver_missing cloudGap entries still partial (monday_round-19 moved to implemented) (got: $MS_PARTIAL_CHECK)"
...
")
assert_ok "$([[ "$IMPL_COUNT" == "4" ]] && echo 0 || echo 1)" \
  "summary.cloudGapImplementedCount == 4 (baserow + clickup + jira + monday) (got: $IMPL_COUNT)"
